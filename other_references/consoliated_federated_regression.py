"""Retain the original consolidated ridge implementation for reference."""
...
from dominate.tags import h1, h2, h3, hr, table, tbody, td, tr
...
def save_results_to_json(global_results, sites):
    """Write global and per-site reference results as JSON."""
    # Save global results
...
def save_results_to_html(output, sites):
    """Write global and per-site reference results as HTML."""
                ...
                with table():
                    with tbody():
                        ...
                        with tr():
                            ...
                            global_tstat.insert(
                                0, GlobalOutputMetricLabels.T_STAT.value
                            )
...
def ignore_nans(X, y):
    """Remove rows containing NaN values from model inputs."""
...
def perform_local_step1(covariates_path, data_path):
    """Fit local models and collect model statistics."""
    # Load data
...
def perform_remote_step1(site_results):
    """Aggregate first-round model statistics across sites."""
    global_results = {}
    ...
    for roi_column in site_results[next(iter(site_results))].keys():
        ...
        for _site, results in site_results.items():
            ...
            if total_sum_coefficients is None:
                total_sum_coefficients = np.array(
                    stats[GlobalOutputMetricLabels.COEFFICIENT.value]
                )
            else:
                total_sum_coefficients += np.array(
                    stats[GlobalOutputMetricLabels.COEFFICIENT.value]
            ...
            all_stats_local.append(
                [
                    {
                        GlobalOutputMetricLabels.COEFFICIENT.value: stats[
                            GlobalOutputMetricLabels.COEFFICIENT.value
                        ],
                        GlobalOutputMetricLabels.T_STAT.value: stats[
                            GlobalOutputMetricLabels.T_STAT.value
                        ],
                        GlobalOutputMetricLabels.P_VALUE.value: stats[
                            GlobalOutputMetricLabels.P_VALUE.value
                        ],
                        GlobalOutputMetricLabels.R_SQUARE.value: stats[
                            GlobalOutputMetricLabels.R_SQUARE.value
                        ],
                        GlobalOutputMetricLabels.COVARIATE_LABELS.value: stats[
                            GlobalOutputMetricLabels.COVARIATE_LABELS.value
                        ],
                        GlobalOutputMetricLabels.SUM_OF_SQUARES_ERROR.value: stats[
                            GlobalOutputMetricLabels.SUM_OF_SQUARES_ERROR.value
                        ],
...
def perform_local_step2(agg_results):
    """Calculate local metrics against the aggregated model."""
...
def perform_remote_step2(site_results):
    """Aggregate second-round metrics into final statistics."""
    ...
    def t_to_p(ts_beta, dof):
        """Return the p-value for each t-statistic of the coefficient vector.
...
def run_federated_ssr():
    """Run the original two-site federated ridge reference workflow."""
    ...
    for site in sites:
        ...
        covariates_path = os.path.join(
            f"../test_data/{site}/{site}_ssr_fsl_repo_covariates.csv"
        )
