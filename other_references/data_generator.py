"""Generate ridge input tables from FreeSurfer aseg files."""
...
def generate_data_csv(
    aseg_file_names, data_dir, output_file_name, freesurfer_labels=None
):
    """Generate a FreeSurfer data CSV for a set of subjects.

    Args:
        aseg_file_names: Files to parse for subject measurements.
        data_dir: Directory containing the aseg files.
        output_file_name: Name of the generated CSV file.
        freesurfer_labels: Optional labels to include in the output.
    """
    freesurfer_labels = freesurfer_labels or []
    log_list = []
        ...
        if file_name:
            try:
                y_ = pd.read_csv(
                    ...
                    sep="\t",
                    header=None,
                    names=["Measure:volume", file_name],
                    index_col=0,
                )
                y_ = y_[~y_.index.str.contains("Measure:volume")]
                if not freesurfer_label_set.issubset(set(y_.index)):
                    err_msg = (
                        f"ERROR: Freesurfer Areas of interest provided in the input are not present in the aseg "
                ...
                repeated_brain_regions = freesurfer_label_set.intersection(
                    y_[y_.index.duplicated()].index.tolist()
                )
                if any(repeated_brain_regions):
                    remove_aseg_file_names.append(file_name)
                    log_list.append(
                        f"SKIPPING file {os.path.join(data_dir, file_name)} which has repeated "
                ...
                log_list.append(
                    f"Empty content in the file {os.path.join(data_dir, file_name)}"
                )
                continue
...
def get_data_csv_from_covariates(
    data_dir,
    covariates_file_name,
    freesurfer_labels=None,
    output_data_file_name="data.csv",
):
    """Generate a data CSV from aseg files listed in a covariates file.

    Args:
        data_dir: Directory containing aseg files and the covariates file.
        covariates_file_name: Name of the covariates file.
        freesurfer_labels: Optional labels to include; all labels are the default.
        output_data_file_name: Name of the generated data file.
    """
    freesurfer_labels = freesurfer_labels or []
    freesurfer_file_names = _get_freesurfer_file_names_from_covariates(
        os.path.join(data_dir, covariates_file_name)
    )
    aseg_file_names_to_remove, logs = generate_data_csv(
        freesurfer_file_names, data_dir, output_data_file_name, freesurfer_labels
    )

    if len(aseg_file_names_to_remove) > 0:
        new_covariates_file_name = os.path.join(
            data_dir, "updated_" + covariates_file_name
        )
        logs.append(
            f"Updating and creating a new covariates file skipping rows with the "
            f"following aseg files: {str(aseg_file_names_to_remove)}"
        )
        temp_df = pd.read_csv(
            os.path.join(data_dir, covariates_file_name),
            header=0,
            index_col="freesurferfile",
        )
