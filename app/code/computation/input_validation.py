"""Validate and normalize ridge regression input data."""
...
def _build_normalized_column_map(
    columns, file_label: str, logger: Logger
) -> Dict[str, str]:
    """Map normalized headers to originals, rejecting ambiguous duplicates."""
    ...
    for column in columns:
        ...
        if normalized_column in normalized_column_map:
            error_message = (
                f"{file_label} contains duplicate headers after normalization: "
...
def _select_columns_case_insensitive(
    dataframe: pd.DataFrame,
    expected_columns: list,
    file_label: str,
    logger: Logger,
) -> pd.DataFrame:
    """Select configured columns case-insensitively and restore canonical names."""
    normalized_column_map = _build_normalized_column_map(
        dataframe.columns, file_label, logger
    )
    missing_columns = [
        column
    ...
    if missing_columns:
        error_message = (
            f"{file_label} headers do not contain all expected headers. Expected at "
...
def validate_and_get_inputs(
    ...
    """Validate covariate and dependent files against computation parameters."""
    try:
        ...
        covariates = _select_columns_case_insensitive(
            covariates, expected_covariates, "Covariates", logger
        )
        data = _select_columns_case_insensitive(
            data, expected_dependents, "Data", logger
        )

        logger.info(
            f"-- Checking covariate and dependent files : {str(covariates_path)}, {str(data_path)}"
        )

        X, y = _convert_data_to_given_type(
            covariates,
            expected_covariates_info,
            data,
            expected_dependents_info,
            logger,
...
def _convert_data_to_given_type(
    ...
    """Convert input columns to the configured data types.

    Subjects with missing data are either removed or rejected according to
    ``ignore_subjects_with_missing_entries``. Strict checking rejects raw
    values whose Python types do not match the configured column type.
    """
    ...
    all_rows_to_ignore = _validate_data_datatypes(
        combined_df, column_info, logger, strict_type_checking
    )
    if len(all_rows_to_ignore) > 0:
        ...
        logger.info(
            f" Data validation passed for all the columns: {str(expected_column_names)}"
        )
    ...
    try:
        for column_name, column_datatype in column_info.items():
            logger.info(
                f"Casting datatype of column: {column_name} to the requested datatype : {column_datatype}"
            )
            if column_datatype.strip().lower() == "int":
                combined_df[column_name] = pd.to_numeric(
                    combined_df[column_name], errors="coerce"
                ).astype("int")  # or .astype('Int64')
                ...
                err_msg = (
                    f"Invalid datatype provided in the input for column : {column_name} and datatype: "
        ...
        curr_rows_to_ignore = combined_df[
            combined_df.isnull().any(axis=1)
        ].index.tolist()
        if len(curr_rows_to_ignore) > 0:
            if ignore_subjects_with_missing_entries:
                logger.info(
                    f"-- Ignored following rows with incorrect column values: {str(_get_user_row_numbers(curr_rows_to_ignore))}"
                )
                combined_df.drop(curr_rows_to_ignore, inplace=True)
            else:
                err_msg = (
                    f"Following rows have empty or invalid entries for columns after converting to their "
    ...
    try:
        for column_name, column_datatype in column_info.items():
            ...
            if strict_type_checking:
                ...
                def _is_wrong_type(
                    ...
                    if expected_type in ("int", "float"):
                        return isinstance(val, bool) or not isinstance(
                            val, expected_types
                        )
                    return not isinstance(val, expected_types)
                ...
                if dtype_key == "int":
                    temp = pd.to_numeric(data_df[column_name], errors="coerce").astype(
                        "int"
                    )
            ...
            if len(rows_to_ignore) > 0:
                logger.info(
                    f"Rows with incorrect values for column {column_name} : {str(_get_user_row_numbers(rows_to_ignore))}"
                )
            else:
                logger.info(
